        ...
        switch (state) {
            case BASIC -> {
                ...
                if (Character.isLetter(current)) {
                    ...
                    switch (value) {
                        case "LIKE" -> {
                            return token = new Token(TokenType.OPERATOR, "LIKE");
                        }
                        case "lastName" -> {
                            return token = new Token(TokenType.FIELD, "lastName");
                        }
                        case "firstName" -> {
                            return token = new Token(TokenType.FIELD, "firstName");
                        }
                        case "jmbag" -> {
                            return token = new Token(TokenType.FIELD, "jmbag");
                        }
                        default -> {
                            if (value.equalsIgnoreCase("AND"))
                                return token = token = new Token(TokenType.LOGICAL, "AND");
                            token = new Token(TokenType.LOGICAL, value);
